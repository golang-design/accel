...
func runBallot(t *testing.T, size, below uint32) ballotOut {
	t.Helper()
	width := int(testkernels.BallotKernel.WorkgroupSize.X)
	o := ballotOut{
	...
	err := kernel.DispatchCooperativeWith(&testkernels.BallotKernel, accel.ID3{X: 1},
		kernelabi.Args{
			Uniforms: []any{testkernels.BallotDims{Below: below}},
...
func TestTheBallotIsRefusedOnMetal(t *testing.T) {
	if testkernels.BallotKernel.MSL != "" {
	...
	if got := testkernels.BallotKernel.Caps; got&want == 0 {
...
func TestTheAuthoredBallotMatchesItsLoweringAtOneLane(t *testing.T) {
	width := int(testkernels.BallotKernel.WorkgroupSize.X)
	d := testkernels.BallotDims{Below: 5}
	...
	kernel.RunAuthored(&testkernels.BallotKernel, kernel.ID3{}, kernel.ID3{X: 1}, 1,
		func(th kernel.Thread) {
			testkernels.Ballot(th, d, a.count, a.lowest, a.lower, a.bit, a.any)
		})

...
func sampleParams() testkernels.Params {
	p := testkernels.Params{
		Scale:  2.5,
...
func TestCodecMatchesTheSpecExample(t *testing.T) {
	if got := testkernels.ParamsBlockSize; got != 96 {
		t.Fatalf("ParamsBlockSize = %d, want 96", got)
	}

	p := sampleParams()
	dst := make([]byte, testkernels.ParamsBlockSize)
	if err := (testkernels.ParamsCodec{}).Encode(dst, p); err != nil {
...
func TestCodecRefusesAShortBuffer(t *testing.T) {
	err := (testkernels.ParamsCodec{}).Encode(make([]byte, 32), sampleParams())
	if err == nil {
...
func TestTransformMatchesAuthored(t *testing.T) {
	for _, n := range []int{0, 1, 24, 32, 40} {
		t.Run(fmt.Sprint(n), func(t *testing.T) {
			...
			runAuthoredKernel(&testkernels.TransformKernel, n, func(th accel.Thread) {
				testkernels.Transform(th, p, in, authored)
			})
...
func TestKernelDeclaresItsUniform(t *testing.T) {
	k := &testkernels.TransformKernel
	if len(k.Uniforms) != 1 {
		t.Fatalf("%d uniforms, want 1", len(k.Uniforms))
	}
	u := k.Uniforms[0]
	if u.Name != "p" || u.Type != "Params" || u.Size != testkernels.ParamsBlockSize {
		t.Errorf("uniform is %+v, want p of Params at %d bytes", u, testkernels.ParamsBlockSize)
	}
...
func TestUniformBufferRoundTrip(t *testing.T) {
	...
	got := make([]byte, testkernels.ParamsBlockSize)
	if err := d.Queue().ReadBuffer(ub.Buffer(), 0, got); err != nil {
		t.Fatalf("ReadBuffer: %v", err)
	}

	want := make([]byte, testkernels.ParamsBlockSize)
	if err := (testkernels.ParamsCodec{}).Encode(want, p); err != nil {
		t.Fatal(err)
	}
	...
	if b := ub.Buffer(); b.DType() != accel.U8 || b.Count() != testkernels.ParamsBlockSize {
		t.Errorf("the uniform buffer is %v of %d, want u8 of %d",
			b.DType(), b.Count(), testkernels.ParamsBlockSize)
	}
...
func TestUniformBufferRejectsAnOversizedBlock(t *testing.T) {
	...
	_, err = accel.NewUniformBuffer[testkernels.Params](d, testkernels.ParamsCodec{})
	if err == nil {

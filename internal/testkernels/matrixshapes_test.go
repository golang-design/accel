...
func sampleMatrixParams() testkernels.MatrixParams {
	var p testkernels.MatrixParams
	for c := range p.Wide {
...
func TestMatrixCodecWritesEveryElementAtItsColumnStride(t *testing.T) {
	if got := testkernels.MatrixParamsBlockSize; got != 32+64+96 {
		t.Fatalf("MatrixParamsBlockSize = %d, want %d", got, 32+64+96)
	}
	p := sampleMatrixParams()
	dst := make([]byte, testkernels.MatrixParamsBlockSize)
	if err := (testkernels.MatrixParamsCodec{}).Encode(dst, p); err != nil {
...
func TestMatrixShapesMatchesAuthored(t *testing.T) {
	...
	runAuthoredKernel(&testkernels.MatrixShapesKernel, n, func(th accel.Thread) {
		testkernels.MatrixShapes(th, p, authored)
	})

	generated := make([]float32, n)
	if err := direct.Run(&testkernels.MatrixShapesKernel,
		direct.Cover(&testkernels.MatrixShapesKernel, n),

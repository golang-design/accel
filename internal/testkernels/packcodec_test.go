...
func TestPackParamsEncodesToStd140(t *testing.T) {
	...
	const scalars = 3 * 4
	const arrays = 2 * testkernels.PackRank * 16
	if got, want := testkernels.PackParamsBlockSize, scalars+4+arrays; got != want {
	...
	p := testkernels.PackParams{Rank: 3, Count: 24, Offset: 5}
	for i := range testkernels.PackRank {
		p.Extent[i] = uint32(100 + i)
		p.Stride[i] = uint32(200 + i)
	}

	dst := make([]byte, testkernels.PackParamsBlockSize)
	if err := (testkernels.PackParamsCodec{}).Encode(dst, p); err != nil {
	...
	if err := (testkernels.PackParamsCodec{}).Encode(dst[:8], p); err == nil {
		t.Error("Encode wrote into a buffer smaller than the block")
	}
	if got := (testkernels.PackParamsCodec{}).EncodedSize(); got != testkernels.PackParamsBlockSize {
		t.Errorf("EncodedSize is %d and the block is %d", got, testkernels.PackParamsBlockSize)
	}
}
...
func TestPackGathersAStridedView(t *testing.T) {
	...
	p := testkernels.PackParams{Rank: 3, Count: d0 * d1 * d2}
	...
	err := kernel.Dispatch(&testkernels.PackKernel, accel.ID3{X: uint32(len(src))},
...
func TestPackAppliesTheOffset(t *testing.T) {
	...
	p := testkernels.PackParams{Rank: 1, Count: 4, Offset: 7}
	p.Extent[0], p.Stride[0] = 4, 2

	dst := make([]float32, 4)
	err := kernel.Dispatch(&testkernels.PackKernel, accel.ID3{X: 4},
		kernelabi.Args{Slices: []any{src, dst}, Uniforms: []any{p}})
	if err != nil {
		t.Fatalf("dispatch: %v", err)
	}

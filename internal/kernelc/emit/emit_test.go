...
func TestGoldenOutput(t *testing.T) {
	...
	golden := filepath.Join("testdata", "kernels.golden")
...
func TestGeneratedSourceTypeChecks(t *testing.T) {
	...
	files := []*ast.File{f}
	entries, err := os.ReadDir(filepath.Join("..", "..", "kernels"))
	if err != nil {
		t.Fatal(err)
	}
	for _, e := range entries {
		...
		authored, err := os.ReadFile(filepath.Join("..", "..", "kernels", name))
		if err != nil {
			t.Fatal(err)
		}
	...
	if _, err := conf.Check("golang.design/x/accel/internal/kernels", fset, files, nil); err != nil {
...
func generateCorpus(t testing.TB) []byte {
	...
	out, err := emit.Generate(emit.Package{Name: "kernels", Kernels: kernels})
	if err != nil {
...
func loadCorpus(t testing.TB) []*ir.Func {
	t.Helper()
	pkgs, err := front.Load(filepath.Join("..", "..", ".."), "golang.design/x/accel/internal/kernels")
	if err != nil {
		t.Fatalf("Load: %v", err)
	}

...
func TestCommittedFileIsFresh(t *testing.T) {
	...
	want := []string{"Add", "AtomicOps", "CountAbove", "CountWorkgroups", "Exchange", "Histogram", "LinearTiled", "MatMulTiled", "MatVec", "Normalize", "RMSNorm", "ReduceLoop", "ReduceSum", "ReduceUnrolled", "Scale", "SegmentSum", "Softmax", "SubgroupReduce", "SubgroupReduceFallback", "Transform"}
...
func copyCorpus(t *testing.T) string {
	...
	repo := root(t)
	mod := "module example.com/kerneltest\n\ngo 1.27\n\n" +

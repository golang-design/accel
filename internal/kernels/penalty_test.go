...
func penalise(t *testing.T, d kernels.PenaltyDims, logits []float32, history []uint32) []float32 {
	t.Helper()
	counts := make([]uint32, d.Vocab)
	out := make([]float32, d.Vocab)
	runFlat(t, &kernels.PenaltyClearKernel, int(d.Vocab),
		kernelabi.Args{Uniforms: []any{d}, Slices: []any{counts}})
	runFlat(t, &kernels.PenaltyCountKernel, len(history),
		kernelabi.Args{Uniforms: []any{d}, Slices: []any{history, counts}})
	runFlat(t, &kernels.PenaltyApplyKernel, int(d.Vocab),
...
func TestTheDivisivePenaltyMovesANegativeLogitDown(t *testing.T) {
	...
	d := kernels.PenaltyDims{Vocab: 2, History: 2, Count: 2, Repetition: 2}
...
func TestOnlyTheFilledPartOfTheHistoryIsCounted(t *testing.T) {
	...
	d := kernels.PenaltyDims{Vocab: 4, History: 8, Count: 3, Presence: 1}
...
func TestAnOutOfRangeHistoryEntryIsDropped(t *testing.T) {
	logits := []float32{0, 0, 0, 0}
	d := kernels.PenaltyDims{Vocab: 4, History: 4, Count: 4, Presence: 1}
...
func TestCountsAreClearedBetweenSteps(t *testing.T) {
	const vocab = 4
	d := kernels.PenaltyDims{Vocab: vocab, History: 2, Count: 2, Presence: 1}
	history := []uint32{1, 1}
	...
	step := func() float32 {
		runFlat(t, &kernels.PenaltyClearKernel, vocab,
			kernelabi.Args{Uniforms: []any{d}, Slices: []any{counts}})
		runFlat(t, &kernels.PenaltyCountKernel, len(history),
			kernelabi.Args{Uniforms: []any{d}, Slices: []any{history, counts}})
		runFlat(t, &kernels.PenaltyApplyKernel, vocab,
			kernelabi.Args{Uniforms: []any{d}, Slices: []any{logits, counts, out}})
		return out[1]
	}
	first := step()
...
func TestAPenaltyIsOneUpdatePerTokenAndNotOnePerOccurrence(t *testing.T) {
	...
	const f = 0.1
	logits := []float32{1}
	d := kernels.PenaltyDims{Vocab: 1, History: 3, Count: 3, Frequency: f}

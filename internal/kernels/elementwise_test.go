...
func TestTheElementwiseFamily(t *testing.T) {
	...
	cases := []struct {
		...
		name: "add",
		run: func(out []float32) {
			runFlat(t, &kernels.ElemAddKernel, n, kernelabi.Args{
				Slices: []any{a, b, out}})
		},
		...
		name: "mul",
		run: func(out []float32) {
			runFlat(t, &kernels.ElemMulKernel, n, kernelabi.Args{
				Slices: []any{a, b, out}})
		},
		want: func(i int) float64 { return float64(a[i]) * float64(b[i]) },
	}, {
		name: "scale",
		run: func(out []float32) {
			runFlat(t, &kernels.ElemScaleKernel, n, kernelabi.Args{
				Slices:   []any{a, out},
				Uniforms: []any{kernels.ScaleParams{Factor: 0.25}},
			})
		},
		...
		name: "silu",
		run: func(out []float32) {
			runFlat(t, &kernels.SiLUKernel, n, kernelabi.Args{Slices: []any{a, out}})
		},
		want: func(i int) float64 {
			x := float64(a[i])
		...
		name: "swiglu",
		run: func(out []float32) {
			runFlat(t, &kernels.SwiGLUKernel, n, kernelabi.Args{
				Slices: []any{a, b, out}})
		},
		want: func(i int) float64 {
			x := float64(a[i])
...
func TestSiLUIsFiniteAtBothExtremes(t *testing.T) {
	...
	runFlat(t, &kernels.SiLUKernel, len(in), kernelabi.Args{Slices: []any{in, out}})
...
func TestGatherRowsChecksItsIDs(t *testing.T) {
	...
	runFlat(t, &kernels.GatherRowsKernel, rows*width, kernelabi.Args{
...
func TestScatterRowsDropsAnOutOfRangeWrite(t *testing.T) {
	...
	runFlat(t, &kernels.ScatterRowsKernel, rows*width, kernelabi.Args{
...
func TestRoPERotatesPairsAndLeavesTheTail(t *testing.T) {
	...
	runFlat(t, &kernels.RoPEKernel, rows*rotary/2, kernelabi.Args{
...
func TestRoPEHonoursThePosition(t *testing.T) {
	...
	base := func(pos uint32) []float32 {
		...
		runFlat(t, &kernels.RoPEKernel, rotary/2, kernelabi.Args{
...
func TestAuthoredElementwiseFamily(t *testing.T) {
	...
	t.Run("add", func(t *testing.T) {
		au := make([]float32, n)
		gen := run(&kernels.ElemAddKernel, kernelabi.Args{Slices: []any{a, b, au}},
			func(th kernel.Thread) { kernels.ElemAdd(th, a, b, au) })
		compareExact(t, au, gen)
	})
	t.Run("mul", func(t *testing.T) {
		au := make([]float32, n)
		gen := run(&kernels.ElemMulKernel, kernelabi.Args{Slices: []any{a, b, au}},
			func(th kernel.Thread) { kernels.ElemMul(th, a, b, au) })
		compareExact(t, au, gen)
	})
	t.Run("scale", func(t *testing.T) {
		p := kernels.ScaleParams{Factor: 0.5}
		au := make([]float32, n)
		gen := run(&kernels.ElemScaleKernel,
			kernelabi.Args{Slices: []any{a, au}, Uniforms: []any{p}},
			func(th kernel.Thread) { kernels.ElemScale(th, p, a, au) })
		compareExact(t, au, gen)
	})
	t.Run("silu", func(t *testing.T) {
		au := make([]float32, n)
		gen := run(&kernels.SiLUKernel, kernelabi.Args{Slices: []any{a, au}},
			func(th kernel.Thread) { kernels.SiLU(th, a, au) })
		compareExact(t, au, gen)
	})
	t.Run("swiglu", func(t *testing.T) {
		au := make([]float32, n)
		gen := run(&kernels.SwiGLUKernel, kernelabi.Args{Slices: []any{a, b, au}},
			func(th kernel.Thread) { kernels.SwiGLU(th, a, b, au) })
		compareExact(t, au, gen)
	})
}
...
func TestAuthoredRowAndRotationKernels(t *testing.T) {
	...
	t.Run("gather", func(t *testing.T) {
		p := kernels.RowParams{Rows: 3, Width: 4, Capacity: 5}
	...
	t.Run("rope", func(t *testing.T) {
		...
		drive(int(p.Rows*p.RotaryDim/2), func(th kernel.Thread) {
			kernels.RoPE(th, p, positions, au)
		})
		gen := append([]float32(nil), start...)
		runFlat(t, &kernels.RoPEKernel, int(p.Rows*p.RotaryDim/2),
...
func TestRoPERotatesEachRowAtItsOwnPosition(t *testing.T) {
	...
	row := func(pos uint32, src []float32) []float32 {
		p := kernels.RoPEParams{Rows: 1, Width: width, RotaryDim: rotary, Base: 10000}
	...
	p := kernels.RoPEParams{Rows: 2, Width: width, RotaryDim: rotary, Base: 10000}
	runFlat(t, &kernels.RoPEKernel, 2*rotary/2, kernelabi.Args{
...
func TestASignedUniformArrivesSigned(t *testing.T) {
	...
	runFlat(t, &kernels.ElemBiasKernel, len(in), kernelabi.Args{
		Uniforms: []any{kernels.BiasParams{Offset: offset}},
		Slices:   []any{in, out},
	})

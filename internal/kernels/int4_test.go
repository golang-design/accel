...
func TestTheInt4KernelsReadAConstantGroupAsTheQuantizerWroteIt(t *testing.T) {
	...
	if err := kernel.DispatchCooperative(&kernels.QuantMatMulInt4Kernel,
		accel.ID3{
			X: (N + kernels.TileN - 1) / kernels.TileN,
			Y: (M + kernels.TileM - 1) / kernels.TileM,
		},
		kernelabi.Args{
			Uniforms: []any{kernels.GEMMDims{M: M, K: K, N: N}},
...
func TestTheAuthoredTiledInt4KernelMatchesItsLowering(t *testing.T) {
	...
	d := kernels.GEMMDims{M: M, K: K, N: N}
	...
	for gy := range groups.Y {
		for gx := range groups.X {
			...
			kernel.RunAuthored(&kernels.QuantMatMulInt4Kernel,
				kernel.ID3{X: gx, Y: gy}, groups, 128, func(th kernel.Thread) {
					kernels.QuantMatMulInt4(th, d, a, packed, scales, zeros,

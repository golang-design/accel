...
func TestSegmentOffsetsArePrefixSums(t *testing.T) {
	for _, c := range []struct {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			if err := direct.Run(&kernels.SegmentOffsetsKernel, accel.ID3{X: 1},
				kernelabi.Args{
					Uniforms: []any{kernels.SegmentDims{Rows: uint32(rows)}},
...
func raggedRef(t *testing.T, d kernels.RaggedDims, q, k, v []float32,
...
func raggedFixture(d kernels.RaggedDims, counts []uint32) (q, k, v []float32,
...
func runRagged(t *testing.T, d kernels.RaggedDims, q, k, v []float32,
	pages, lengths, offsets []uint32) []float32 {
	...
	if err := kernel.DispatchCooperative(&kernels.AttentionRaggedKernel,
...
func TestARaggedSequenceShorterThanItsCountKeepsItsMask(t *testing.T) {
	...
	if err := kernel.DispatchCooperative(&kernels.AttentionRaggedF16Kernel,
...
func TestTheAuthoredRaggedKernelMatchesItsLowering(t *testing.T) {
	...
	for g := range groups.X {
		...
		kernel.RunAuthored(&kernels.AttentionRaggedKernel, kernel.ID3{X: g},
			groups, 128, func(th kernel.Thread) {
				kernels.AttentionRagged(th, d, q, k, v, pages, lengths,
...
func TestTheAuthoredSegmentOffsetsMatchesItsLowering(t *testing.T) {
	...
	dims := kernels.SegmentDims{Rows: uint32(len(counts))}

	authored := make([]uint32, len(counts)+1)
	kernel.RunAuthored(&kernels.SegmentOffsetsKernel, kernel.ID3{X: 0},
		kernel.ID3{X: 1, Y: 1, Z: 1}, 1, func(th kernel.Thread) {
			kernels.SegmentOffsets(th, dims, counts, authored)
		})
...
func TestTheAuthoredF16RaggedKernelMatchesItsLowering(t *testing.T) {
	...
	for g := range groups.X {
		var scores, red [128]float32
		kernel.RunAuthored(&kernels.AttentionRaggedF16Kernel, kernel.ID3{X: g},
			groups, 128, func(th kernel.Thread) {
				kernels.AttentionRaggedF16(th, d, q, narrowK, narrowV, pages,
...
func TestAnF16RaggedTokenPastTheLastSegmentIsPadding(t *testing.T) {
	...
	for g := range groups.X {
		...
		kernel.RunAuthored(&kernels.AttentionRaggedF16Kernel, kernel.ID3{X: g},
			groups, 128, func(th kernel.Thread) {
				kernels.AttentionRaggedF16(th, d, q, k16, v16, pages, lengths,
					short, authored, &scores, &red)
			})
	}
	for i := range authored {

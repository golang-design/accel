...
func linearRef(d kernels.LinearDims, q, k, v, alpha, beta []float32,
...
func linearFixture(d kernels.LinearDims, counts []uint32) (q, k, v, alpha, beta,
...
func runLinear(t *testing.T, d kernels.LinearDims, q, k, v, alpha, beta,
	state []float32, offsets []uint32) []float32 {

	t.Helper()
	total := offsets[d.Batch]
	out := make([]float32, total*d.Heads*d.ValueDim)
	if err := direct.Run(&kernels.LinearAttentionKernel,
...
func linearDims() kernels.LinearDims {
	return kernels.LinearDims{Batch: 2, Heads: 2, KeyDim: 6, ValueDim: 4, GateHeads: 1}
}
	...
	for _, gateHeads := range []uint32{1, 2} {
		t.Run(fmt.Sprintf("gateHeads=%d", gateHeads), func(t *testing.T) {
			...
			for g := range groups.X {
				kernel.RunAuthored(&kernels.LinearAttentionKernel, kernel.ID3{X: g},
					groups, 128, func(th kernel.Thread) {
						kernels.LinearAttention(th, d, q, k, v, alpha, beta, offsets,
...
// kernel's contract.
func chunkedLinearRef(d kernels.LinearDims, q, k, v, alpha, beta,
...
func TestTheChunkedFormEqualsTheSequentialKernel(t *testing.T) {
	d := kernels.LinearDims{Batch: 2, Heads: 2, KeyDim: 6, ValueDim: 4, GateHeads: 1}
	counts := []uint32{7, 5}

...
func TestTiledGEMMMatchesItsReference(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(fmt.Sprintf("%dx%dx%d", c.m, c.n, c.k), func(t *testing.T) {
			...
			err := kernel.DispatchCooperative(&kernels.MatMulTiledKernel, groups,
				kernelabi.Args{
					Slices:   []any{a, b, out},
					Uniforms: []any{dims},
				})
...
func TestAuthoredTiledGEMM(t *testing.T) {
	...
	for gy := range groups.Y {
		for gx := range groups.X {
			...
			kernel.RunAuthored(&kernels.MatMulTiledKernel, kernel.ID3{X: gx, Y: gy}, groups, 128,
				func(th kernel.Thread) {
					kernels.MatMulTiled(th, dims, a, b, authored, &tileA, &tileB)
				})
		}
	}

...
func TestSegmentSumMatchesAuthored(t *testing.T) {
	for _, n := range []int{0, 1, 7, 32, 33, 100} {
		t.Run(fmt.Sprint(n), func(t *testing.T) {
			...
			runAuthoredKernel(&kernels.SegmentSumKernel, n, func(th accel.Thread) {
				kernels.SegmentSum(th, in, authored)
			})

			generated := make([]float32, n)
			if err := direct.Run(&kernels.SegmentSumKernel,
				direct.Cover(&kernels.SegmentSumKernel, n),
...
func TestCountAboveMatchesAuthored(t *testing.T) {
	for _, n := range []int{0, 1, 16, 17, 40} {
		t.Run(fmt.Sprint(n), func(t *testing.T) {
			...
			runAuthoredKernel(&kernels.CountAboveKernel, n, func(th accel.Thread) {
				kernels.CountAbove(th, in, authored)
			})

			generated := make([]int32, n)
			if err := direct.Run(&kernels.CountAboveKernel,
				direct.Cover(&kernels.CountAboveKernel, n),
				kernelabi.Args{Slices: []any{in, generated}}); err != nil {
				t.Fatalf("direct.Run: %v", err)
			}
...
func TestNormalizeMatchesAuthored(t *testing.T) {
	for _, n := range []int{0, 1, 32, 33, 70} {
		t.Run(fmt.Sprint(n), func(t *testing.T) {
			...
			runAuthoredKernel(&kernels.NormalizeKernel, n, func(th accel.Thread) {
				kernels.Normalize(th, in, authored, authoredScratch)
			})

			generated := make([]float32, n)
			generatedScratch := make([]float32, n)
			if err := direct.Run(&kernels.NormalizeKernel,
				direct.Cover(&kernels.NormalizeKernel, n),

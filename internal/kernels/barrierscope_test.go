...
func TestAStorageBarrierPublishesToTheWorkgroup(t *testing.T) {
	const groups = 3
	width := int(kernels.PublishStorageKernel.WorkgroupSize.X)

	scratch := make([]uint32, groups)
	out := make([]uint32, groups*width)
	if err := kernel.DispatchCooperative(&kernels.PublishStorageKernel,
		accel.ID3{X: groups},
...
func TestASharedBarrierOrdersTheSharedArray(t *testing.T) {
	const groups = 2
	width := int(kernels.PublishSharedKernel.WorkgroupSize.X)

	out := make([]uint32, groups*width)
	if err := kernel.DispatchCooperative(&kernels.PublishSharedKernel,
		accel.ID3{X: groups},
...
func TestEachBarrierScopeLowersToItsOwnMask(t *testing.T) {
	for _, c := range []struct {
		...
		{
			&kernels.PublishSharedKernel,
			"threadgroup_barrier(mem_flags::mem_threadgroup)",
			"mem_device",
		},
		// Storage only.
		{
			&kernels.PublishStorageKernel,
...
func TestAMaskedBarrierIsASuspensionPoint(t *testing.T) {
	for _, c := range []struct {
		...
		{&kernels.PublishStorageKernel, 1},
		{&kernels.PublishSharedKernel, 1},
	} {
...
func TestASubgroupBarrierPublishesWithinItsSubgroup(t *testing.T) {
	width := int(kernels.SubgroupPublishKernel.WorkgroupSize.X)

	for _, size := range []uint32{1, 4, 32, 64} {
		t.Run(fmt.Sprintf("size=%d", size), func(t *testing.T) {
			out := make([]float32, width)
			err := kernel.DispatchCooperativeWith(&kernels.SubgroupPublishKernel,
...
func TestASubgroupBarrierLowersToSubgroupScope(t *testing.T) {
	src := kernels.SubgroupPublishKernel.MSL
	if src == "" {
...
func TestSubgroupsMaySitAtDifferentSubgroupBarriers(t *testing.T) {
	width := int(kernels.SubgroupStaggerKernel.WorkgroupSize.X)
	...
	for _, size := range []uint32{4, 16, 64} {
		t.Run(fmt.Sprintf("size=%d", size), func(t *testing.T) {
			out := make([]float32, width)
			err := kernel.DispatchCooperativeWith(&kernels.SubgroupStaggerKernel,
				accel.ID3{X: 1}, kernelabi.Args{Slices: []any{out}},
				kernel.Options{Diagnostics: true, SubgroupSize: size})
			if err != nil {
...
func TestTheAuthoredBarrierScopeKernelsMatchTheirLowerings(t *testing.T) {
	t.Run("PublishStorage", func(t *testing.T) {
		...
		for g := range uint32(groups) {
			kernel.RunAuthored(&kernels.PublishStorageKernel, kernel.ID3{X: g},
				kernel.ID3{X: groups}, 128, func(th kernel.Thread) {
					kernels.PublishStorage(th, scratch, authored)
				})
		}
	...
	t.Run("PublishShared", func(t *testing.T) {
		...
		for g := range uint32(groups) {
			var sh [32]uint32
			kernelabi.Poison(sh[:])
			kernel.RunAuthored(&kernels.PublishSharedKernel, kernel.ID3{X: g},
				kernel.ID3{X: groups}, 128, func(th kernel.Thread) {
					kernels.PublishShared(th, authored, &sh)
				})
		}
	...
	for _, c := range []struct {
		...
		{"SubgroupPublish", &kernels.SubgroupPublishKernel,
			func(th kernel.Thread, out []float32, sh *[64]float32) {
				kernels.SubgroupPublish(th, out, sh)
			}},
		{"SubgroupStagger", &kernels.SubgroupStaggerKernel,
			func(th kernel.Thread, out []float32, sh *[64]float32) {
				kernels.SubgroupStagger(th, out, sh)
			}},
	} {

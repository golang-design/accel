...
func groupedFixture(d kernels.GroupedDims, counts []uint32) (x, w []float32,
	offsets []uint32) {
...
func runGrouped(t *testing.T, d kernels.GroupedDims, x, w []float32,
	offsets []uint32) []float32 {

	t.Helper()
	tokens := offsets[d.Experts]
	out := make([]float32, tokens*d.N)
	if err := kernel.DispatchCooperative(&kernels.GroupedMatVecKernel,
...
func TestAGroupedProductMatchesEachExpertAlone(t *testing.T) {
	d := kernels.GroupedDims{Experts: 3, K: 64, N: 5}
	counts := []uint32{2, 1, 3}
...
func TestAGroupedProductAcceptsAnEmptyExpert(t *testing.T) {
	d := kernels.GroupedDims{Experts: 4, K: 64, N: 3}
...
func TestAGroupedProductUsesTheExpertItLookedUp(t *testing.T) {
	d := kernels.GroupedDims{Experts: 2, K: 64, N: 2}
...
func TestTheAuthoredGroupedKernelMatchesItsLowering(t *testing.T) {
	d := kernels.GroupedDims{Experts: 3, K: 64, N: 4}
	...
	for g := range groups.X {
		var sh [128]float32
		kernel.RunAuthored(&kernels.GroupedMatVecKernel, kernel.ID3{X: g},
			groups, 128, func(th kernel.Thread) {
				kernels.GroupedMatVec(th, d, x, w, offsets, authored, &sh)
			})
	}
...
func TestAGroupedTokenPastTheLastExpertIsPadding(t *testing.T) {
	...
	for g := range groups.X {
		...
		kernel.RunAuthored(&kernels.GroupedMatVecKernel, kernel.ID3{X: g},
			groups, 128, func(th kernel.Thread) {
				kernels.GroupedMatVec(th, d, x, w, short, authored, &sh)
			})
	}
	for i := range authored {
...
func runGroupedTiled(t *testing.T, d kernels.GroupedTiledDims, x, w []float32,
	offsets []uint32) []float32 {

	t.Helper()
	out := make([]float32, d.Tokens*d.N)
	if err := kernel.DispatchCooperative(&kernels.GroupedMatMulKernel,
		accel.ID3{X: (d.N + kernels.TileN - 1) / kernels.TileN, Y: d.Experts},
		kernelabi.Args{
			Uniforms: []any{d},
			Slices:   []any{x, w, offsets, out},
		}); err != nil {
...
func TestATiledGroupedProductClampsCountsPastTheRows(t *testing.T) {
	dv := kernels.GroupedDims{Experts: 2, K: 40, N: 20}
...
func TestTheAuthoredTiledGroupedKernelMatchesItsLowering(t *testing.T) {
	dv := kernels.GroupedDims{Experts: 3, K: 40, N: 18}
	x, w, offsets := groupedFixture(dv, []uint32{5, 0, 9})
	total := offsets[dv.Experts]
	d := kernels.GroupedTiledDims{
		Experts: dv.Experts, Tokens: total, K: dv.K, N: dv.N,
	}

	groups := kernel.ID3{
		X: (d.N + kernels.TileN - 1) / kernels.TileN, Y: d.Experts, Z: 1,
	}
	...
	for gy := range groups.Y {
		for gx := range groups.X {
			...
			kernel.RunAuthored(&kernels.GroupedMatMulKernel,
				kernel.ID3{X: gx, Y: gy}, groups, 128, func(th kernel.Thread) {
					kernels.GroupedMatMul(th, d, x, w, offsets, authored,
						&tileA, &tileB)
				})
		}
	}

...
func argmax(t *testing.T, logits []float32) uint32 {
	t.Helper()
	out := make([]uint32, 1)
	err := kernel.DispatchCooperative(&kernels.SampleArgmaxKernel, accel.ID3{X: 1},
		kernelabi.Args{
			Slices: []any{logits, out},
			Uniforms: []any{kernels.SampleDims{
...
func categorical(t *testing.T, probs []float32, draw float32) uint32 {
	t.Helper()
	out := make([]uint32, 1)
	err := kernel.Dispatch(&kernels.SampleCategoricalKernel, accel.ID3{X: 1},
		kernelabi.Args{
...
func TestSamplingFollowsTheDistribution(t *testing.T) {
	...
	err := kernel.DispatchCooperative(&kernels.SoftmaxKernel, accel.ID3{X: rows},
		kernelabi.Args{
			Slices:   []any{logits, probs},
			Uniforms: []any{kernels.RowDims{Rows: rows, Width: vocab}},
		})
	if err != nil {
...
func TestABatchedSampleDrawsIndependentlyPerRow(t *testing.T) {
	...
	err := kernel.Dispatch(&kernels.SampleCategoricalKernel, accel.ID3{X: rows},
		kernelabi.Args{
			Slices:   []any{probs, draws, out},
			Uniforms: []any{kernels.SampleDims{Vocab: vocab, Rows: rows}},
		})
	if err != nil {
		t.Fatalf("dispatch: %v", err)
	}
...
func TestABatchedArgmaxIsPerRow(t *testing.T) {
	...
	out := make([]uint32, rows)
	err := kernel.DispatchCooperative(&kernels.SampleArgmaxKernel, accel.ID3{X: rows},
		kernelabi.Args{
			Slices:   []any{logits, out},
			Uniforms: []any{kernels.SampleDims{Vocab: vocab, Rows: rows}},
		})

...
func TestTheIntegerReductionsMatchAScalarLoop(t *testing.T) {
	width := int(kernels.IntReduceKernel.WorkgroupSize.X)
	...
	for _, size := range []uint32{1, 4, 32, 64} {
		t.Run(fmt.Sprintf("size=%d", size), func(t *testing.T) {
			...
			err := kernel.DispatchCooperativeWith(&kernels.IntReduceKernel,
				accel.ID3{X: 1},
...
func TestTheAuthoredIntegerReductionsMatchTheirLoweringAtOneLane(t *testing.T) {
	...
	kernel.RunAuthored(&kernels.IntReduceKernel, kernel.ID3{}, kernel.ID3{X: 1}, 1,
		func(th kernel.Thread) {
			kernels.IntReduce(th, in, aMinI, aMaxI, aMinU, aMaxU)
		})
...
func TestTheBitwiseReductionsMatchAScalarLoop(t *testing.T) {
	width := int(kernels.BitReduceKernel.WorkgroupSize.X)
	...
	for _, size := range []uint32{1, 4, 32, 64} {
		t.Run(fmt.Sprintf("size=%d", size), func(t *testing.T) {
			...
			err := kernel.DispatchCooperativeWith(&kernels.BitReduceKernel,
				accel.ID3{X: 1},
...
func TestTheAuthoredBitwiseReductionsMatchTheirLoweringAtOneLane(t *testing.T) {
	...
	kernel.RunAuthored(&kernels.BitReduceKernel, kernel.ID3{}, kernel.ID3{X: 1}, 1,
		func(th kernel.Thread) {
			kernels.BitReduce(th, in, aA, aO, aX, aAU, aOU, aXU)
		})
...
func TestTheProductReductionsMatchAScalarLoop(t *testing.T) {
	width := int(kernels.MulReduceKernel.WorkgroupSize.X)
	...
	for _, size := range []uint32{1, 4, 32, 64} {
		t.Run(fmt.Sprintf("size=%d", size), func(t *testing.T) {
			...
			err := kernel.DispatchCooperativeWith(&kernels.MulReduceKernel,
				accel.ID3{X: 1},
...
func TestTheAuthoredProductsMatchTheirLoweringAtOneLane(t *testing.T) {
	...
	kernel.RunAuthored(&kernels.MulReduceKernel, kernel.ID3{}, kernel.ID3{X: 1}, 1,
		func(th kernel.Thread) {
			kernels.MulReduce(th, inF, inI, aF, aI, aU)
		})

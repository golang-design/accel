...
func TestPairAverageMatchesAuthored(t *testing.T) {
	if k := kernels.PairAverageKernel; len(k.Bindings) != 2 ||
	...
	for _, n := range []int{0, 1, 7, 32, 33, 100} {
		t.Run(fmt.Sprint(n), func(t *testing.T) {
			...
			runAuthoredKernel(&kernels.PairAverageKernel, n, func(th accel.Thread) {
				kernels.PairAverage(th, in, authored)
			})

			generated := make([]float32, n)
			if err := direct.Run(&kernels.PairAverageKernel,
				direct.Cover(&kernels.PairAverageKernel, n),

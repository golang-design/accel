...
func topK(t *testing.T, w []float32, k int) []float32 {
	...
	err := kernel.DispatchCooperative(&kernels.TopKMaskKernel, accel.ID3{X: 1},
		kernelabi.Args{
			Slices: []any{w, out},
			Uniforms: []any{kernels.TopDims{
...
func topP(t *testing.T, w []float32, p float32) []float32 {
	t.Helper()
	out := make([]float32, len(w))
	err := kernel.DispatchCooperative(&kernels.TopPMaskKernel, accel.ID3{X: 1},
		kernelabi.Args{
			Slices: []any{w, out},
			Uniforms: []any{kernels.TopDims{
...
// [kernels.SampleArgmax] has always used.
//
...
func TestTruncationBatchesByRow(t *testing.T) {
	...
	batched := func(k *accel.Kernel, d kernels.TopDims, rows ...[]float32) [][]float32 {
		t.Helper()
		vocab := len(rows[0])
	...
	t.Run("TopKMask", func(t *testing.T) {
		got := batched(&kernels.TopKMaskKernel, kernels.TopDims{K: 2}, rowA, rowB)
	...
	t.Run("TopPMask", func(t *testing.T) {
		got := batched(&kernels.TopPMaskKernel, kernels.TopDims{P: 0.5}, rowA, rowB)
		for i, row := range [][]float32{rowA, rowB} {
			want := topP(t, row, 0.5)
			for j := range want {
				if got[i][j] != want[j] {

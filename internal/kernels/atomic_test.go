...
func TestAtomicsReturnThePreviousValue(t *testing.T) {
	...
	if err := direct.Run(&kernels.AtomicOpsKernel, accel.ID3{X: 1},
...
func TestHistogramCountsEveryElement(t *testing.T) {
	...
	if err := direct.Run(&kernels.HistogramKernel, direct.Cover(&kernels.HistogramKernel, n),
...
func TestAtomicKernelsAreInferredOrderDependent(t *testing.T) {
	for _, k := range []*kernelabi.Kernel{
		&kernels.HistogramKernel,
		&kernels.AtomicOpsKernel,
		&kernels.AtomicOpsI32Kernel,
		&kernels.AtomicAddF32Kernel,
		&kernels.CountWorkgroupsKernel,
	} {
...
func TestAnAtomicMarksItsBindingReadWrite(t *testing.T) {
	...
	for i := range kernels.HistogramKernel.Bindings {
		if b := &kernels.HistogramKernel.Bindings[i]; b.Name == "counts" {
			counts = b
		}
	}
	if counts == nil {
...
func TestTheFloatAtomicIsACapability(t *testing.T) {
	if kernels.HistogramKernel.Caps != 0 {
		t.Errorf("Histogram uses only integer atomics and requires %d", kernels.HistogramKernel.Caps)
	}
}
...
func TestAuthoredAtomicKernels(t *testing.T) {
	...
	t.Run("Histogram", func(t *testing.T) {
		...
		for g := range groups {
			for l := range uint32(group) {
				...
				kernels.Histogram(th, in, counts)
			}
		}
		total := uint32(0)
		for _, c := range counts {
		...
		if err := direct.Run(&kernels.HistogramKernel, accel.ID3{X: groups},
	...
	t.Run("AtomicOps", func(t *testing.T) {
		...
		kernels.AtomicOps(th, authoredState, authoredPrev)
...
func TestTheSignedAtomicsArriveSigned(t *testing.T) {
	...
	if err := direct.Run(&kernels.AtomicOpsI32Kernel, accel.ID3{X: 1},
		kernelabi.Args{Slices: []any{state, prev}}); err != nil {
		t.Fatalf("run: %v", err)
	}
...
func TestTheFloatAtomicReachesAKernel(t *testing.T) {
	...
	if err := direct.Run(&kernels.AtomicAddF32Kernel, accel.ID3{X: 1},
		kernelabi.Args{Slices: []any{state, prev}}); err != nil {
		t.Fatalf("run: %v", err)
	}

...
func TestTheF32GEMMMatchesTheF16OneExactly(t *testing.T) {
	for _, c := range []struct{ m, n, k int }{
		...
		t.Run(fmt.Sprintf("%dx%dx%d", c.m, c.n, c.k), func(t *testing.T) {
			...
			dims := kernels.GEMMDims{M: uint32(c.m), N: uint32(c.n), K: uint32(c.k)}
			groups := accel.ID3{
				X: uint32((c.n + kernels.TileN - 1) / kernels.TileN),
				Y: uint32((c.m + kernels.TileM - 1) / kernels.TileM),
				Z: 1,
			}
			narrow := make([]float32, c.m*c.n)
			wide := make([]float32, c.m*c.n)
			if err := kernel.DispatchCooperative(&kernels.MatMulTiledKernel, groups,
				kernelabi.Args{Slices: []any{a16, b16, narrow}, Uniforms: []any{dims}}); err != nil {
				t.Fatalf("f16 dispatch: %v", err)
			}
			if err := kernel.DispatchCooperative(&kernels.MatMulTiledF32Kernel, groups,
	...
	for _, c := range []struct{ qHeads, kvHeads, headDim, kvLen uint32 }{
		...
		t.Run(fmt.Sprintf("q%d_kv%d_d%d_len%d", c.qHeads, c.kvHeads, c.headDim, c.kvLen),
			func(t *testing.T) {
				...
				if err := kernel.DispatchCooperative(&kernels.AttentionDecodeKernel, groups,
					kernelabi.Args{
						Slices: []any{q, kf, vf, lengths, wide}, Uniforms: []any{d},
					}); err != nil {
					t.Fatalf("f32 dispatch: %v", err)
				}
				if err := kernel.DispatchCooperative(&kernels.AttentionDecodeF16Kernel, groups,
					kernelabi.Args{
...
func TestTheF16PrefillMatchesTheF32OneExactly(t *testing.T) {
	for _, c := range []struct{ qHeads, kvHeads, headDim, qSeq, base uint32 }{
		...
		t.Run(name, func(t *testing.T) {
			...
			if err := kernel.DispatchCooperative(&kernels.AttentionPrefillKernel, groups,
				kernelabi.Args{
					Slices: []any{q, k32, v32, lengths, wide}, Uniforms: []any{d},
				}); err != nil {
				t.Fatalf("f32 dispatch: %v", err)
			}
			if err := kernel.DispatchCooperative(&kernels.AttentionPrefillF16Kernel, groups,
				kernelabi.Args{
					Slices: []any{q, k16, v16, lengths, narrow}, Uniforms: []any{d},
				}); err != nil {
				t.Fatalf("f16 dispatch: %v", err)
...
func TestTheF16PagedDecodeMatchesTheF32OneExactly(t *testing.T) {
	for _, c := range []struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			wide := make([]float32, len(q))
			narrow := make([]float32, len(q))
			if err := kernel.DispatchCooperative(&kernels.AttentionDecodePagedKernel, groups,
				kernelabi.Args{
...
func TestTheF16ScatterMatchesTheF32OneExactly(t *testing.T) {
	...
	if err := kernel.Dispatch(&kernels.ScatterRowsKernel, groups,
		kernelabi.Args{Slices: []any{in32, ids, state32}, Uniforms: []any{p}}); err != nil {
		t.Fatalf("f32 dispatch: %v", err)
	}
	if err := kernel.Dispatch(&kernels.ScatterRowsF16Kernel, groups,
...
func TestAnF16CacheIsWrittenByKernelsAndThenRead(t *testing.T) {
	...
	scatter := func(base, n uint32, srcOff int) {
		p := kernels.RowParams{Rows: n, Width: width, Capacity: capacity}
		ids := make([]uint32, n)
		for i := range ids {
			ids[i] = base + uint32(i)
		}
		...
		for _, c := range []struct {
			...
			kern := &kernels.ScatterRowsKernel
			if c.f16 {
				kern = &kernels.ScatterRowsF16Kernel
			}
	...
	if err := kernel.DispatchCooperative(&kernels.AttentionDecodeF16Kernel,
		accel.ID3{X: qHeads}, kernelabi.Args{
...
func TestTheNewVariantsMatchTheirAuthoredForms(t *testing.T) {
	t.Run("MatMulTiledF32", func(t *testing.T) {
		...
		for gy := range groups.Y {
			for gx := range groups.X {
				...
				kernel.RunAuthored(&kernels.MatMulTiledF32Kernel, kernel.ID3{X: gx, Y: gy}, groups,
					kernels.TileN*kernels.TileM, func(th kernel.Thread) {
						kernels.MatMulTiledF32(th, d, a, b, authored, &tileA, &tileB)
					})
			}
		}

		generated := make([]float32, m*n)
		err := kernel.DispatchCooperative(&kernels.MatMulTiledF32Kernel,
	...
	t.Run("AttentionDecodeF16", func(t *testing.T) {
		...
		for g := range groups.X {
			...
			kernel.RunAuthored(&kernels.AttentionDecodeF16Kernel, kernel.ID3{X: g}, groups, 128, func(th kernel.Thread) {
				kernels.AttentionDecodeF16(th, d, q, k16, v16, lengths, authored,
					&scores, &red)
			})
		}

		generated := make([]float32, qHeads*headDim)
		err := kernel.DispatchCooperative(&kernels.AttentionDecodeF16Kernel,
			accel.ID3{X: qHeads},
			kernelabi.Args{
...
// # Why compare the two rather than compute a reference twice
//
// [kernels.MatMulTiledF32F16] is [kernels.MatMulTiled] with the
...
func TestTheMixedGEMMMatchesTheF16OneExactly(t *testing.T) {
	for _, c := range []struct{ m, n, k int }{
		...
		t.Run(fmt.Sprintf("%dx%dx%d", c.m, c.n, c.k), func(t *testing.T) {
			...
			if err := kernel.DispatchCooperative(&kernels.MatMulTiledF32F16Kernel, groups,
...
func TestTheF32ActivationQuantizedKernelsMatchTheirF16Forms(t *testing.T) {
	...
	for _, c := range []struct{ m, n, k int }{
		...
		t.Run(fmt.Sprintf("%dx%dx%d", c.m, c.n, c.k), func(t *testing.T) {
			...
			bs := make([]accel.Float16, (len(bq)+kernels.QuantBlock-1)/kernels.QuantBlock)
			for i := range bs {
			...
			narrow := make([]float32, c.m*c.n)
			wide := make([]float32, c.m*c.n)
			wg := int(kernels.QuantMatMulKernel.WorkgroupSize.X)
			flat := accel.ID3{X: uint32((c.m*c.n + wg - 1) / wg), Y: 1, Z: 1}
			if err := kernel.Dispatch(&kernels.QuantMatMulKernel, flat,
				kernelabi.Args{
					Slices: []any{a16, bq, bs, narrow}, Uniforms: []any{dims},
				}); err != nil {
				t.Fatalf("f16 GEMM dispatch: %v", err)
			}
			if err := kernel.Dispatch(&kernels.QuantMatMulF32Kernel, flat,
				kernelabi.Args{
			...
			if err := kernel.DispatchCooperative(&kernels.QuantMatVecKernel, cols,
				kernelabi.Args{
					Slices: []any{a16, bq, bs, narrowVec}, Uniforms: []any{dims},
				}); err != nil {
				t.Fatalf("f16 matvec dispatch: %v", err)
			}
			if err := kernel.DispatchCooperative(&kernels.QuantMatVecF32Kernel, cols,
				kernelabi.Args{
...
func TestTheBF16WideningIsExact(t *testing.T) {
	...
	wg := int(kernels.CastBF16ToF32Kernel.WorkgroupSize.X)
	groups := accel.ID3{X: uint32((len(in) + wg - 1) / wg), Y: 1, Z: 1}
	if err := kernel.Dispatch(&kernels.CastBF16ToF32Kernel, groups,

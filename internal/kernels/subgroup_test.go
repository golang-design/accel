		...
		t.Run(fmt.Sprintf("size=%d", width), func(t *testing.T) {
			for _, n := range []int{group, group * 3, group*2 + 17} {
				t.Run(fmt.Sprintf("n=%d", n), func(t *testing.T) {
					...
					err := kernel.DispatchCooperativeWith(&kernels.SubgroupReduceKernel,
					...
					err = kernel.Dispatch(&kernels.SubgroupReduceFallbackKernel,
						accel.ID3{X: uint32((n + group - 1) / group)},
...
func TestAuthoredSubgroupKernels(t *testing.T) {
	...
	for l := range uint32(group) {
		th := kernel.NewThreadWithSubgroup(kernel.ID3{X: l}, kernel.ID3{X: l},
			kernel.ID3{}, size, kernel.ID3{X: 1}, width)
		kernels.SubgroupReduceFallback(th, in, authored, []uint32{width})
	}
	...
	for l := range uint32(group) {
		...
		kernels.SubgroupReduce(th, in, perLane)
	}
	for i := range perLane {
		if perLane[i] != in[i] {
...
func TestTheSubgroupKernelDeclaresItsCapabilities(t *testing.T) {
	caps := accel.Capability(kernels.SubgroupReduceKernel.Caps)
	...
	if got := kernels.SubgroupReduceFallbackKernel.Caps; got != 0 {
...
func TestTheShuffleSweepAgreesWithTheFallback(t *testing.T) {
	...
	for _, width := range []uint32{1, 4, 32, 64} {
		t.Run(fmt.Sprintf("size=%d", width), func(t *testing.T) {
			...
			err := kernel.DispatchCooperativeWith(&kernels.SubgroupShuffleMixKernel,
			...
			err = kernel.Dispatch(&kernels.SubgroupShuffleMixFallbackKernel,
				accel.ID3{X: 1},
				kernelabi.Args{Slices: []any{in, viaFallback, []uint32{width}}})
			if err != nil {
				t.Fatalf("fallback: %v", err)
...
func TestAuthoredShuffleKernel(t *testing.T) {
	...
	for l := range uint32(group) {
		th := kernel.NewThreadWithSubgroup(kernel.ID3{X: l}, kernel.ID3{X: l},
			kernel.ID3{}, size, kernel.ID3{X: 1}, 1)
		kernels.SubgroupShuffleMix(th, in, authored)
	}
...
func TestTheShuffleKernelDeclaresItsCapabilities(t *testing.T) {
	caps := accel.Capability(kernels.SubgroupShuffleMixKernel.Caps)
	for _, want := range []struct {
		cap  accel.Capability
		name string
		why  string
	...
	if got := kernels.SubgroupShuffleMixFallbackKernel.Caps; got != 0 {
		t.Errorf("the fallback requires capabilities %d and should require none", got)
	}
}
...
func TestTheScanSweepAgreesWithTheFallback(t *testing.T) {
	...
	for _, width := range []uint32{1, 4, 32, 64} {
		t.Run(fmt.Sprintf("size=%d", width), func(t *testing.T) {
			...
			err := kernel.DispatchCooperativeWith(&kernels.SubgroupScanKernel,
			...
			err = kernel.Dispatch(&kernels.SubgroupScanFallbackKernel, accel.ID3{X: 1},
...
func TestAuthoredScanKernel(t *testing.T) {
	...
	for l := range uint32(group) {
		th := kernel.NewThreadWithSubgroup(kernel.ID3{X: l}, kernel.ID3{X: l},
			kernel.ID3{}, size, kernel.ID3{X: 1}, 1)
		kernels.SubgroupScan(th, in, authoredIncl, authoredExcl)
	}
...
func TestTheScanKernelDeclaresItsCapabilities(t *testing.T) {
	caps := accel.Capability(kernels.SubgroupScanKernel.Caps)
	...
	if got := kernels.SubgroupScanFallbackKernel.Caps; got != 0 {
		t.Errorf("the fallback requires capabilities %d and should require none", got)
	}
}

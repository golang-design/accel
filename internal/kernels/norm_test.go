...
func runRow(t *testing.T, k *accel.Kernel, d kernels.RowDims, slices []any) {
	t.Helper()
...
func TestRMSNormMatchesItsReference(t *testing.T) {
	for _, width := range []int{1, 63, 128, 129, 500, 1024} {
		t.Run(fmt.Sprintf("width=%d", width), func(t *testing.T) {
			const rows = 3
			d := kernels.RowDims{Rows: rows, Width: uint32(width), Eps: 1e-5}
			...
			runRow(t, &kernels.RMSNormKernel, d, []any{x, w, out})
...
func TestSoftmaxMatchesItsReference(t *testing.T) {
	for _, width := range []int{1, 63, 128, 129, 500} {
		t.Run(fmt.Sprintf("width=%d", width), func(t *testing.T) {
			const rows = 3
			d := kernels.RowDims{Rows: rows, Width: uint32(width)}
			...
			out := make([]float32, rows*width)
			runRow(t, &kernels.SoftmaxKernel, d, []any{x, out})
...
func TestSoftmaxSurvivesLargeLogits(t *testing.T) {
	const width = 64
	d := kernels.RowDims{Rows: 1, Width: width}
...
func TestRMSNormHandlesAZeroRow(t *testing.T) {
	const width = 64
	d := kernels.RowDims{Rows: 1, Width: width, Eps: 1e-5}
	x := make([]float32, width)
	w := make([]float32, width)
	for i := range w {
		w[i] = 1
...
func TestAuthoredRowKernels(t *testing.T) {
	...
	drive := func(k *accel.Kernel, run func(th kernel.Thread, sh *[128]float32)) {
		var sh [128]float32
		kernelabi.Poison(sh[:])
		kernel.RunAuthored(k, kernel.ID3{}, kernel.ID3{X: 1}, kernels.RowWidth,
	...
	t.Run("RMSNorm", func(t *testing.T) {
		...
		drive(&kernels.RMSNormKernel, func(th kernel.Thread, sh *[128]float32) {
			kernels.RMSNorm(th, d, x, w, authored, sh)
		})

		generated := make([]float32, width)
		runRow(t, &kernels.RMSNormKernel, d, []any{x, w, generated})
	...
	t.Run("Softmax", func(t *testing.T) {
		...
		authored := make([]float32, width)
		drive(&kernels.SoftmaxKernel, func(th kernel.Thread, sh *[128]float32) {
			kernels.Softmax(th, d, x, authored, sh)
		})

		generated := make([]float32, width)
		runRow(t, &kernels.SoftmaxKernel, d, []any{x, generated})
		for i := range authored {
			if math.Abs(float64(authored[i]-generated[i])) > 1e-5 {
				t.Fatalf("element %d: authored %v, generated %v",
					i, authored[i], generated[i])

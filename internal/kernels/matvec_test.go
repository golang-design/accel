...
func TestMatVecAgreesWithTheTiledGEMM(t *testing.T) {
	for _, c := range []struct{ n, k int }{
		...
		t.Run(fmt.Sprintf("N=%d,K=%d", c.n, c.k), func(t *testing.T) {
			...
			dims := kernels.GEMMDims{M: 1, N: uint32(c.n), K: uint32(c.k)}

			viaMatVec := make([]float32, c.n)
			err := kernel.DispatchCooperative(&kernels.MatVecKernel,
			...
			err = kernel.DispatchCooperative(&kernels.MatMulTiledKernel,
				accel.ID3{
...
func TestLinearIsTheGEMMPlusItsBias(t *testing.T) {
	for _, c := range []struct{ m, n, k int }{
		...
		t.Run(fmt.Sprintf("%dx%dx%d", c.m, c.n, c.k), func(t *testing.T) {
			...
			if err := kernel.DispatchCooperative(&kernels.LinearTiledKernel, groups,
...
func TestTheBiasIsBroadcastAlongRows(t *testing.T) {
	...
	err := kernel.DispatchCooperative(&kernels.LinearTiledKernel,
...
func TestAuthoredMatVecAndLinear(t *testing.T) {
	t.Run("MatVec", func(t *testing.T) {
		...
		dims := kernels.GEMMDims{M: 1, N: n, K: k}
		...
		for col := range uint32(n) {
			...
			kernel.RunAuthored(&kernels.MatVecKernel, kernel.ID3{X: col}, kernel.ID3{X: n},
				kernels.RowWidth, func(th kernel.Thread) {
					kernels.MatVec(th, dims, a, b, authored, &sh)
				})
		}

		generated := make([]float32, n)
		if err := kernel.DispatchCooperative(&kernels.MatVecKernel, accel.ID3{X: n},
	...
	t.Run("LinearTiled", func(t *testing.T) {
		...
		kernel.RunAuthored(&kernels.LinearTiledKernel, kernel.ID3{}, groups, 128, func(th kernel.Thread) {
			kernels.LinearTiled(th, dims, a, b, bias, authored, &tileA, &tileB)
		})

		generated := make([]float32, m*n)
		if err := kernel.DispatchCooperative(&kernels.LinearTiledKernel,
			accel.ID3{X: 1, Y: 1, Z: 1},

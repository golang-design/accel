...
func TestLocalsSurviveTheSuspension(t *testing.T) {
	...
	args := kernelabi.Args{Slices: []any{in, out}}
	if err := kernel.DispatchCooperative(&kernels.ExchangeKernel,
...
func TestACooperativeKernelRefusesTheFlatPath(t *testing.T) {
	err := kernel.Dispatch(&kernels.ExchangeKernel, accel.ID3{X: 1},
	...
	for g := range groups {
		for l := range uint32(group) {
			th := kernel.NewThread(kernel.ID3{X: g*group + l},
				kernel.ID3{X: l}, kernel.ID3{X: g}, size, count)
			kernels.Add(th, a, b, authored)
		}
	}

	generated := make([]float32, n)
	if err := direct.Run(&kernels.AddKernel, accel.ID3{X: groups},
...
func TestTheLoopedAndUnrolledReductionsAgree(t *testing.T) {
	...
	looped := run(&kernels.ReduceLoopKernel)
	unrolled := run(&kernels.ReduceUnrolledKernel)
	for i := range looped {
...
func TestTheLoopedReductionCountsItsSuspensions(t *testing.T) {
	if got := kernels.ReduceLoopKernel.Suspensions; got != 2 {
	...
	if got := kernels.ReduceUnrolledKernel.Suspensions; got != 7 {
...
func TestAuthoredReductionsMatchTheirLowerings(t *testing.T) {
	...
	for g := range uint32(groups) {
		...
		for range 7 {
			for l := range uint32(group) {
				th := kernel.NewThread(kernel.ID3{X: g*group + l},
					kernel.ID3{X: l}, kernel.ID3{X: g}, size, count)
				kernels.ReduceUnrolled(th, in, authored, &sh)
			}
		}
	}
	for g := range groups {
	...
	if err := kernel.DispatchCooperative(&kernels.ReduceUnrolledKernel,
...
func TestSharedExtentsReachTheRecord(t *testing.T) {
	for _, c := range []struct {
		...
		{&kernels.ExchangeKernel, []int{64}},
		{&kernels.ReduceLoopKernel, []int{64}},
		{&kernels.ReduceUnrolledKernel, []int{64}},
	} {

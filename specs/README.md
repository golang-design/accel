...
| [050-barrier-scopes.md](050-barrier-scopes.md) | Implemented | 002's first successor: `BarrierShared`/`BarrierStorage`/`SubgroupBarrier`, and the uniformity level a subgroup barrier is checked at |
...
| [056-cpu-intersector.md](056-cpu-intersector.md) | Drafted | 053's third child: the reference binned-SAH BVH, watertight edges, and the deliberate shuffle that makes the order rule checkable |
| [057-metal-ray-tracing.md](057-metal-ray-tracing.md) | Drafted | 053's fourth child: `MTLAccelerationStructure` and `intersection_query` through purego, and the differential against the reference |
| [058-ballot.md](058-ballot.md) | Implemented | 002's fourth successor: `Ballot` and its opaque mask, and the first kernel-visible capability Metal does not have |
| [059-subgroup-reductions.md](059-subgroup-reductions.md) | Implemented | 002's fifth successor: the seventeen reductions §5.2 promises, the frame carriers they need, and the one that needs a numeric bound first |
...
| [062-backend-parity.md](062-backend-parity.md) | In progress | The CPU/Metal agreement matrix: every enumeration member and every tensor operator, with a completeness gate that needs no device and runs on all three platforms |

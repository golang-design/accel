...
| [039-sampling-policy.md](039-sampling-policy.md) | In progress | Post-v0: temperature, penalties, the composition order, and a seeded stream that makes a whole sequence reproducible rather than one token. The stream, the penalty kernels and the composed policy are built; §9's assertion list and the token differential are outstanding, and four deviations are recorded |

...
| `linear_attention_chunked` | **not registered** | — | **The chunked parallel form only** — the sequential one is registered above as `linear_attention` since 2026-08-26. Reassociating the recurrence so a block of tokens is processed with matrix products, with only the block boundaries sequential, is what makes linear attention fast rather than merely correct. It has its own summation order, so its numeric bound is derived against the sequential kernel rather than measured. Original text: a gated-delta / SSM recurrence, which is three of every four layers in the hybrid models the open-weights frontier has moved to (accel issue 17). **In scope**, and two things were said to be prior to a kernel: the recurrence is sequential in $t$, so a prefill is a chunked *scan* rather than a batch of independent rows; and the state is matrix-valued **per sequence**, not per position, which [007](007-tensor-layer.md)'s `State` cannot express — see [043](043-per-row-values.md) §9. Not scheduled: the consumer's dense path is unblocked and this decides whether a 27B hybrid target is reachable, not whether they can build. |

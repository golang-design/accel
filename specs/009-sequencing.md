...
shipped (`internal/kernels/subgroup.go:100,146,183`,
...
- **The Linux coverage gate, twice.** The Metal differential inflates
  `internal/kernels` on darwin, so a kernel whose *authored* form nothing
...
| the allocation timing test read a ratio of 3.5 on an allocator that reads 1.1, because it took the quietest small batch and the quietest large one from *different* rounds and divided those | a full `go test ./...` on a machine above a load average of 200, which is the only condition that separates the two moments enough to matter | the test's own comment already had the argument — interleave, so a spike lands on both sizes and cancels. The code interleaved the measurements and then divided across rounds, throwing the pairing away before the division. **A comment describing a design is not the design.** The guard it lacked was any way to tell a slow allocator from an unmeasurable machine; each round now brackets the large batch with two timings of the same small one and is discarded when they disagree. That guard is needed in both directions, because the minimum selects for whatever noise flattered the allocator: one observed round read 9.6µs at 1,000 live against 777ns at 10,000 and would have won it at 0.08 |
| `LinearAttention` took one gate per token, so a gated delta network — where each value head decays at its own rate — was inexpressible, and 48 of a target model's 64 layers with it | a consumer's conformance test binding the shape the model needs, which accel refused with a message naming per-token and per-sequence when the answer was the third reading | the paragraph that made the decision, [047](047-linear-attention.md) §4, said the shape was "[043](043-per-row-values.md) §2 applied without argument". **A claim that announces it was not argued with is one nobody argued with**, and the counter-argument was three sections up in the same file: §1's recurrence carries a head index on every term except the two gates. 043 now asks which axes the *kernel's parallel unit* carries rather than whether a value differs per token |
| a local holding an `ID3` lowered its components as struct fields, which Metal refuses as an illegal vector component | writing the first kernel that binds an id to a local, four milestones after the emitter shipped | the emitter asked the *expression* whether it was an id — an intrinsic call — so `t.GroupID().X` was right and `g := t.GroupID(); g.X` was wrong. The property belongs to the **type**, and a type-shaped question answered by matching one expression form is right exactly as long as nobody writes the other. Latent for four milestones because no corpus kernel had |
| `RunAuthored` took the workgroup extent as an argument, so every one of its 30 call sites wrote a literal beside a comparison against that same kernel's generated form | reading it while adding an accessor that returns the extent | the comparison is the only thing that would notice, and it is what breaks: a differential running the authored function at one width and its lowering at another compares two programs and calls them equal wherever the shape does not matter. **A test's own fixture is not covered by the test.** It reads the kernel now, and forcing the two apart fails four differentials |
| the uniformity lattice's middle level had no seed, so every subgroup-scope rule was vacuous — a predicate on `SubgroupLane` read as workgroup-uniform and a barrier under it was accepted | writing the discriminating pair [002](002-compute-model.md) §12 already names, as a test, and watching three of its four rejections pass | the level existed, was named in the type's doc, was ordered correctly, and had a `String` method with a test — everything except a value that reaches it. **A lattice with an unreachable level is a two-level lattice with extra prose**, and nothing could have caught it until a rule needed the distinction, because until then both readings gave the same answer |
| a mutation of the new subgroup arrival check passed every test, because the only kernel using a subgroup barrier put it at the top level — where every lane is at it and a workgroup-wide check passes anyway | reinstating the fault rather than reading the test | the assertion's *subject* was unreachable from the corpus. This is the third variant of "unreachable concluded from one construction path" this file records, and the first where the unreachable thing was a code path in the runtime rather than a refusal in an operator. The fix is a kernel that staggers its subgroups with a subgroup-uniform loop bound |
| five new kernels shipped with no authored/lowering differential, and the Linux coverage gate failed at exactly 90.0% | CI, on the platform where the Metal differential is not there to cover the lowering | **the second time this exact class has appeared**, and the row above it already describes the first. What is new is that the gate reports a *percentage* rather than a name, so the signal arrives as "the package moved 0.4%" — the fix is [010](010-kernel-corpus.md) §6's obligation, which was written down and simply not applied to five kernels in a row |
| a ballot was specified in [002](002-compute-model.md) §5.2, had its mask type, its `Thread` method, its IR opcode, its scheduler case and its capability constant — and no intrinsic entry, so no kernel could call any of it | writing [058](058-ballot.md) and listing what existed against what a kernel can reach | the fourth instance of *built and unreachable* this file records, after `pagetable`, `Contiguous`, and the sampling kernels. What is new is the proportion: **five of six pieces were built**, so every reading of the code said the feature was there. A table of "where it lives" against "can a kernel reach it" is what made the one missing row visible, and it took a column nothing generates |
| `RenderPass.SetTexture` was accepted on a Metal device — resolved, checked against the stage's declared slots, checked for feedback — and then dropped, because only `internal/cpu` ever read the field it filled | asking, during a prioritisation pass, which shipped exported declarations reach nothing on which backend | **a validated binding is not a bound one.** Four checks ran and passed on the way to a value nobody read, and each of them made the path look more implemented than it was. The CPU/Metal differential could not see it either: no graphics test bound a texture on a Metal device, so the oracle was comparing a path the GPU never took. The general form is narrower than *built and unreachable* above — here the caller **can** reach it, and one backend of two silently answers with zeros. What finds it is asking of each exported declaration not "does it work" but "which backends read what it writes" |
| `Stage.Discards` was declared in the IR, copied into the generated record, declared again in the stage record, and honoured by the rasterizer — and the one line that sets it did not exist, so the field was permanently false | building the intrinsic, and grepping first for what was already there | the **fifth** instance of *built and unreachable*, and the highest proportion yet: four of six pieces, including the consuming branch. That is what made it read as a missing feature rather than as an unreachable one, and the two want different work — the first is a design, the second is a line. The check that separates them is the same column [058](058-ballot.md) needed: where each piece lives, against whether anything *assigns* the value the rest read |
| accel's `ColorWriteMask` numbers its channels red-first and `MTLColorWriteMask` numbers them alpha-first, and the Metal backend passed the value through as `int(m)`, so every mask arrived mirrored | writing the differential the corpus already required, after predicting the fault from the cast alone | **the default value is the one that hides a wrong mapping.** `WriteAll` is 0xF under either numbering, and it was the only mask any test had used; a mask symmetric about the middle is its own mirror, so one of the five cases written for this test passes *against the bug*. A partial fixture would have reported success. The rule `metalLoadAction` already states — write the mapping out by name, because a numeric conversion mistranslates the day the two enumerations diverge — understates it: these two never agreed |
| a generated *decoder* was emitted for every uniform block, including the ones only a compute dispatch takes, where nothing can ever call it — and it dropped `internal/kernels` below the Linux coverage gate | CI, on the platform where the Metal differential is not there to cover a lowering | **the built-and-unreachable class arriving by the generator rather than by hand.** A by-value parameter is written and never read back, so a decoder for one is code with no caller by construction — and generating it is a decision nobody makes explicitly, which is what makes it easy. The emitter now emits a decoder only for a block a *stage* takes. The same red also exposed twenty-one codecs and six authored stages whose only caller was the darwin differential, which is [010](010-kernel-corpus.md) §6's obligation unapplied for the fourth time |
| a test helper defined in a `_darwin_test.go` file and called from a portable one — twice in one day | CI, both times, on a change whose own tests passed locally | `go build` does not compile tests, so the cross-target *build* loop CONTRIBUTING already prescribed cannot see it and a cross-target **vet** loop can. The guard existed and was one verb short. It costs seconds, and it is now beside the build loop with what it catches written down |

...
- ~~**§2.3 and §2.5 — the barrier call set**~~ — **closed 2026-08-28 by [050](050-barrier-scopes.md)**: BarrierShared, BarrierStorage and SubgroupBarrier are Thread methods, IR ops, intrinsics and MSL cases. When this row was written, only Thread.Barrier existed (internal/kernel/kernel.go:136, intrin.go:398). BarrierShared, BarrierStorage and SubgroupBarrier appear nowhere in the tree — no Thread method, no IR op, no intrinsic table entry, no emitter case. §2.5's four-row table, the §2.3 Subgroup-scope barrier row, and §2.4's stor
- ~~**§2.5 — what Barrier makes visible, on Metal**~~ — **closed 2026-08-27**; the lowering is `mem_threadgroup | mem_device`, which is §2.5's own table. When this row was written, msl.go:1042 emitted `threadgroup_barrier(mem_flags::mem_threadgroup)` unconditionally. `mem_device` occurs nowhere in the repository. So the one barrier that does exist orders shared memory only, while §2.5 makes it normative that Barrier covers shared *and* storage. specs/022-ms
- ~~**§1.3 — three built-in id accessors**~~ — **closed 2026-08-28 by [052](052-dispatch-shape.md)**. t.WorkgroupSize(), t.NumGroups() and t.GlobalSize() are in §1.3's table and in §3.3's workgroup-uniform seed set, and none of the three exists on internal/kernel.Thread or in intrin.go's table. §1.3's t.SubgroupID() and t.SubgroupInvocationID() shipped under different names (SubgroupIndex at subgrou
- ~~**§6.2 — float-to-integer conversion**~~ — **closed 2026-08-27 by [051](051-float-to-int.md)**, as `kmath.ToI32`/`ToU32` with the bare conversion refused by name; 051 §2.1 records why two rather than four and why `kmath`. When this row was written, accel.ToI32, accel.ToU32, accel.ToI8 and accel.ToU8 did not exist anywhere. internal/kernelc/front/build.go:1024 conversion() accepts any convertible pair and emits ir.NewConvert with no float-source check, so `int32(f)` compiles. The whole saturating/NaN-to-zero contract, and the bit-identical-on-ar
- ~~**§5.2 — the ballot value type**~~ — **closed 2026-08-28 by [058](058-ballot.md)**: `accel.KernelMask` is the alias, `t.SubgroupBallot` has its intrinsic, and the five methods lower. When this row was written, there was no accel.KernelMask. The mask type is internal/kernel.Mask re-exported only as kernelabi.Mask (kernelabi/kernelabi.go:75), so a caller cannot name it from the public package. Thread.SubgroupBallot (subgroup.go:327) exists on the scheduler side with no intrinsic, which the header notes; the 
...
- ~~**§8 — composed operator budgets**~~ — **the machinery and three of the six items are built 2026-08-29, [008](008-numerics.md) §12**: forward sensitivity propagation, the budget trace with per-operator attribution, the singularity refusal, and RMSNorm, Softmax and MatMul/Linear. It immediately found `RMSNorm`'s tensor assertion two orders loose. **Composed Attention, the golden model's DAG, and the fused-kernel rule are outstanding** and are compositions over the same operators rather than new machinery. When this row was written:  no refusal when an interval crosses a singularity, and no per-operator attribution for RMSNorm, Softmax, MatMul/Linear, composed Attention, or the golden model. Only §7's reduction/dot-product form and §8.1's interpolation bound exist. The spec na
...
- ~~**3.1 Interpolation**, the integer half~~ — **closed 2026-08-29 by [032](032-stage-abi.md) §15**: `accel:"flat"` parsed from the field tag, an untagged uninterpolable field refused with its position, the packers bitcasting, `[[flat]]` in MSL, and `raster.State.Flat` set at last. **`noperspective` landed the same day, [032](032-stage-abi.md) §16**, so §3.1 is built in full. When this row was written: No interpolation qualifiers exist. There is no `flat` or `noperspective` tag handling in the front end or either emitter, and no rejection of an integer varying. Worse than absent: emit/emit.go:416-430 stageFlatten appends every varyings field straight into a []float32, so an authored integer varyin
- ~~**3.2 The varyings slot limit**~~ — **closed 2026-08-29 by [032](032-stage-abi.md) §14**: `mslabi.StageVaryingSlotLimit`, checked at generation with the position, refused at sixteen slots and accepted at fifteen. §14 also resolves §3.2's own contradiction about where the limit comes from. When this row was written: No slot computation and no limit. Nothing computes ceil(components/4) per field, no device limit is consulted at generation, and no error names the field list, the count and the limit.
...
- ~~**4.2 Discard**~~ — **closed 2026-08-29 by [032](032-stage-abi.md) §13**: `OpDiscard`, the intrinsic, both lowerings, and the CPU rasterizer's branch reached at last — four of the six pieces already existed and the field was permanently false, which is why this read as missing rather than as unreachable. §13.1 records the spelling deviation and §13.2 why the depth half is asserted through the pipeline rather than by a readback that does not exist. When this row was written: There is no accel.Discard intrinsic, no IR node, and no lowering. ir.Func.Discards is never set, so kernel.Stage.Discards is always false and no backend decision depends on it.
...
- ~~**§2.1, §2.2 — stencil**~~ — **the CPU half is closed 2026-08-29 by [033](033-render-api.md) §10**: the state, the per-face masks and ops, `SetStencilReference`, the creation refusal, and `Depth32FloatStencil8` — which had to come first, because no format could carry a stencil aspect and `Depth24PlusStencil8` is refused for a reason that stands. **Metal refuses a stencil draw by name** and is the remaining slice. When this row was written:  No stencil ops, read/write masks, per-face state or dynamic reference value is exposed. internal/raster/pipeline.go:64-137 implements all of it, but nothing in the accel API can reach it. §2.2's creation-time refusal for stenci
...
- ~~**§4.1 and §6 — the recorded uniform offset channel**~~ — **closed 2026-08-30 by [033](033-render-api.md) §11**: `SetVertexUniformBuffer`/`SetFragmentUniformBuffer`, `Device.UniformStride`, a generated std140 decoder for the CPU side, and the N-object replay frame. When this row was written: No call binds a BufferView to a stage uniform. render_feedback_test.go:221 TestADrawCanBeParameterisedByAUniformBuffer t.Skip()s naming exactly this. The N-object replay frame does not exist. (The spec's Outstanding table admits this one.)
- ~~**§6 — the `Clear` without `LoadClear` refusal**~~ — **closed 2026-08-29**: refused at record with the attachment and the load op named. A *zero* clear value is exempt and cannot not be — it is the field's zero value, so "cleared to black" and "said nothing" are the same bytes; the accepting half is tested for that reason. When this row was written: Recorder.RenderPass (render.go:545-622) never inspects ColorAttachment.Clear or DepthAttachment.Clear against the load op; build.go:448 appends c.Clear unconditionally. A clear value set with LoadKeep or LoadDontCare is silently ignored, which is the exact hazard §3.1 says must be an error.
...
- ~~§7 Done bullet: "feedback is rejected for an overlapping subresource and **accepted** for a disjoint mip and a disjoint array layer, which is the half a handle~~ — **the accepting half self-activated 2026-08-30**, §10.4: it was written to run the day a second mip became admissible, and it does. Its skip is removed with the refusal it watched for. When this row was written: — no test — the accepting half skips. render_feedback_test.go:154 TestADisjointSubresourceIsNotFeedback calls t.Skipf as soon as NewTexture with MipLevels: 2 is refused (045 §8.3 still refuses it), so t
...
- ~~**§7 corpus row: "Origin agreement, discriminating form"**~~ — **closed 2026-08-30 by [035](035-cpu-rasterizer.md) §12.1**, with the fetch as a fragment one because [032](032-stage-abi.md) §5.1 refuses a texture in a compute kernel; both assertions confirmed to catch different faults. When this row was written: The two-path test (path A: compute texel fetch of (x,0) then a buffer read; path B: host texture read of row 0; assert A == B and that both hold the top-row value) does not exist. internal/raster/raster_test.go:283 TestRowZeroIsTheTopRow checks the rasterizer's own window mapping only — one path, no
- ~~**§7 corpus row: "Handoff stays on device"**~~ — **closed 2026-08-30, §12.2**: two nodes, no copy node, a hazard between them, and the values so it is not vacuous. When this row was written: No test inspects a deferred graph's nodes for the absence of a host transfer between a geometry pass and a tonemap. No such graph is built anywhere in the tree.
- ~~**§7 corpus row: "Depth readback through a transfer node"**~~ — **closed 2026-08-30, §12.3**: the direct read refused and the recorded copy landing the predicted window depth. When this row was written: No test reads a depth attachment back through a transfer node. The macOS private-depth constraint this row exists to cover is unverified.
- ~~**§7 corpus row: "Per-object replay"**~~ — **closed 2026-08-30, [033](033-render-api.md) §11.4**, on both backends. §11.4 records that the first fixture passed with every draw reading offset zero. When this row was written: N objects at recorded uniform offsets, submitted twice. Blocked by 033 §4.1's unbuilt channel; render_feedback_test.go:221 skips with that reason.
- ~~**§7 corpus row: "Determinism"**~~ — **closed 2026-08-29, [035](035-cpu-rasterizer.md) §11.2**: one built graph submitted twice, compared bit for bit, on both backends. When this row was written: No test submits the same render graph twice on one backend and compares the images. TestPlansAreDeterministic (internal/conformance) is about plan-cache identity, not pixels.
...
- ~~**§7.1 — the differential's coverage**~~ — **mostly closed 2026-08-29 by [035](035-cpu-rasterizer.md) §11**: the write mask, MRT, per-attachment blend and determinism now run on both backends, and the write mask was **wrong on Metal** — `MTLColorWriteMask` numbers its channels from alpha, so every mask arrived mirrored and only `WriteAll` and masks symmetric about the middle survived. **Stencil remains CPU-only** and is unreachable from the public API ([033](033-render-api.md) §2.1). When this row was written: "Every corpus entry that produces pixels runs on the CPU backend **and** on Metal". The MRT, per-attachment blend, write-mask, stencil and discard entries live only in internal/raster/pipeline_test.go and internal/cpu/render_test.go, both CPU-side. The Metal differential (render_darwin_test.go) cove
...
- ~~**§5.3 "Metal's ABI is refusing callers on devices that are not Metal"**~~ — **closed 2026-08-30 by [042](042-surface-completion.md) §10.1**: `Limits.MaxVertexBuffers`, reported per device, and the refusal names the device and its number. When this row was written: Limits has no MaxVertexBuffers field (limits.go). vertexlayout.go:132 still refuses >16 vertex buffers on every device from the mslabi.StageVertexBufferLimit constant, and the error does not name a device number.
- ~~**§5.3 "The CPU oracle is setting the public API's ceiling"**~~ — **closed 2026-08-30, §10.2**: eight normalized integer formats with their conversions stated, compared exactly across both backends. The snorm clamp is the assertion with teeth -- removing it makes the oracle report -1.007874 against Metal's -1. When this row was written: No normalized integer vertex attribute formats. AttrFormat is {AttrFloat32, x2, x3, x4} only (vertexlayout.go:38-44) and the doc comment still gives the oracle-parity reason this section rejects. No unorm8/snorm16 conversion is stated anywhere.
- ~~**§3.1 Uniform buffers: keep, and connect**~~ — **closed 2026-08-30, [033](033-render-api.md) §11**: `UniformBuffer.View` has a consumer, and `TestADrawCanBeParameterisedByAUniformBuffer` asserts the stage reads the block rather than skipping. When this row was written: No draw or dispatch takes a BufferView as a stage uniform. UniformBuffer.View (uniform.go:177) has no consumer; render_feedback_test.go:238 skips with that reason. §7's bullet 2 additionally still says "a dispatch reads a UniformBuffer at a recorded offset", which §3.1's 2026-08-24 correction replac
...
- ~~**§2.1 Texture attachments / texel fetch (the Metal half)**~~ — **closed 2026-08-29 by [045](045-texture-attachments.md) §8.6**: `mtl.NewSampledTexture`, `RenderEncoder.SetVertexTexture`/`SetFragmentTexture`, and a staged copy before the encoder opens; a fragment fetch and a vertex fetch are each compared against the oracle. When this row was written: The binding half exists only on the CPU backend. build.go:846-900 fills driver.RenderDraw.VertexTextures/FragmentTextures; only internal/cpu/exec.go:654-658 reads them. `grep -rn Textures internal/metal internal/mtl` returns nothing, and internal/kernelc/emit/mslstage.go:162 still emits `texture2d<f

**Unbacked (code exists, nothing checks it):**

- ~~§7: "no exported declaration is reachable-and-unusable without a refusal naming what is missing" — the spec's whole thesis.~~ — **the counterexample is closed 2026-08-29**: Metal binds a stage texture, and the drop was confirmed before the fix rather than inferred — 240 of 240 floats differed and Metal raised nothing. Originally: Falsified by RenderPass.SetTexture on a Metal device. build.go:860-900 accepts the binding and lowers it into driver.RenderDraw.FragmentTextures; internal/metal reads that field nowhere and refuses no
...
- ~~**§4 stages row / §7 bullet 4 — texel fetch on Metal**~~ — **closed 2026-08-29, [045](045-texture-attachments.md) §8.6.** When this row was written: internal/metal never binds a stage texture (no setFragmentTexture:/setVertexTexture: selector in internal/mtl/render_darwin.go, no read of driver.RenderDraw.FragmentTextures anywhere in internal/metal), and nothing refuses the case. The MSL emitter does declare the argument (internal/kernelc/emit/ms
...
- ~~**§4 resources row / §7 bullet 1 — MipLevels and ArrayLayers**~~ — **closed 2026-08-30 by [045](045-texture-attachments.md) §10**: the allocation sums every level, the hazard range and both extents follow the view, and the copy names the base subresource explicitly. Giving the copy entry points a *view* is what remains, and it is an API change rather than a layout one. When this row was written: texturealloc.go:76-85 still refuses both above one. §8.3 records the reason (textureBytes sizes one level, texture-buffer copies move one, and a recorded access covers the whole byte range), and none of those three layers has changed.
...
- ~~§7 bullet 1: "an attachment names a TextureView, and MipLevels/ArrayLayers above one are accepted rather than refused"~~ — **closed 2026-08-30, §10**; `TestMipsAboveTheBaseLevelAreRefused` is replaced by tests of the layout and of two mips written and read independently. When this row was written: Half of it is unimplemented: texturealloc.go:76-85 refuses both, TestMipsAboveTheBaseLevelAreRefused (texture_test.go:613) asserts the refusal. §8.3 records the deviation but the §7 Done list was left
...
- ~~**Barrier storage-class masks and the Metal device-memory barrier**~~ — **written as [050](050-barrier-scopes.md) and built 2026-08-28.** §2.3, §2.4, §2.5 and §3.1 at subgroup scope. Add BarrierShared, BarrierStorage and SubgroupBarrier as Thread methods, IR ops and intrinsics; give the CPU scheduler a per-class rendezvous so a masked barrier orders only the class it names; fix emit/msl.go:1042 to emit mem_threadgr
...
| --- | --- | --- | --- |
| 1 | [002](002-compute-model.md) §2.5 | ~~`Barrier` is normative over shared **and** storage; Metal emits `mem_threadgroup` only~~ — **closed 2026-08-27**, the lowering now matches §2.5's table. **[050](050-barrier-scopes.md)'s masked variants landed 2026-08-28**, with `SubgroupBarrier`, so the row is closed entirely | [050](050-barrier-scopes.md) |
| 2 | [002](002-compute-model.md) §6.2 | ~~a saturating float-to-int contract that does not exist~~ — **the hazard is closed 2026-08-27**: `kmath.ToI32`/`ToU32` on both backends, the three stages migrated, the bare conversion refused by name. **closed entirely 2026-08-27**, including the CPU-versus-Metal boundary differential this row said was outstanding — `SaturatingConvert` runs it over NaN, both infinities and the exact limits with no ULP ceiling. This sentence was stale from the day it was written | [051](051-float-to-int.md) |
